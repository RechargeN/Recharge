...
2. принят отдельный ADR по участию `timeFit` в ранжировании — **ожидается**;
3. завершён активный slice стабилизации — **ожидается**.

Реализация в приложении не входит в текущее документальное изменение и
начинается только отдельным пользовательским запросом после оставшихся gates.
...
До принятия нового ADR:

- `timeFitWeight = 0`;
- фильтрация и badges реализуются;
- default ordering остаётся `geoFreshness`.

После принятия ADR вес берётся из config и включается kill switch:
...
Документ не подменяет ADR: конкретный разрешённый вес фиксируется только
Accepted ADR. Деление на travel minutes и ненормализованные сигналы
запрещены.
...
| U2 | timeWindow задан до нового ADR | Hard filter работает, ranking weight = 0 |
