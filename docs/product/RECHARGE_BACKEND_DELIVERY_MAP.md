...
## 5. Реестр проектных документов — 22 (v2.4.51)

| ID | Файл | Accountable owner | Уникальная область | Основные зависимости | Spec | Runtime |
|---|---|---|---|---|---|---|
| BCK-01 | `RECHARGE_BACKEND_MASTER_SPEC.md` | Platform Architecture | Target architecture, module boundaries, shared invariants | Accepted ADR, BCK-02, §3 anchors | Review v0.4.47 — Present | Local R0 tooling scaffold only; product/cloud Absent |
| BCK-02 | `RECHARGE_BACKEND_DELIVERY_MAP.md` | Architecture owner | Registry, ownership, dependencies, waves, risks and gates | Current repository facts | Approved v2.4 baseline; factual registry v2.4.51 | N/A |
| BCK-03 | `BACKEND_API_CONTRACT_STANDARD.md` | API Platform | Envelopes, typed errors, versioning, pagination, idempotency, event envelope, schema evolution, minimum client | BCK-01, API Contracts Workflow, OD-09 | Draft v0.3.5 — Present; Booking-v1 named decisions and bounded parity evidence recorded | Absent |
...
BCK-09 v1.9 и его 22/22 coverage matrix v1.8 существуют как Review-документы:
...
ECL03-D12, BCK09-API-CORR-01 и BCK09-API-PAR-01 закрывают parent semantics,
command artifact и bounded query/hash contract parity. Перед `Approved` всё
ещё нужны девять независимых verdicts из BCK09-REV-01 v0.8;
BCK09-API-REV-01 v0.6 сохраняет Hold для Node 22, callable raw-body/runtime
feasibility и independent evidence. Перед runtime также необходимы отдельно
Approved ECL-03C v1.7 и все
...
1. Ревизия v2.4.51 содержит 22 BCK-specs и 6 runbooks.
...
Формальный prerequisite выполнен: `RECHARGE_BACKEND_MASTER_SPEC.md` v0.4.47
...
3. BCK-03 v0.3.5, BCK-04 v0.4.16, BCK-05 v0.2.23 и BCK-20 v0.2.2
