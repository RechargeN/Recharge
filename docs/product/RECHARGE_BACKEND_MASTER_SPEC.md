...
| Event Booking | ADR 0019 Accepted; ECL-03 v1.3/D01–D12 Approved/Accepted; BCK-09 v1.7 and coverage v1.6 Review/Present with 22/22 coverage; BCK09 API target selected; BCK09-REV-01 v0.6 / API review v0.4 narrowed Hold; all nine signatures Pending; ECL-03B contracts/domain Done; BCK09-API-CORR-01 v0.3 command correction Implemented/Review; ECL-03C v1.5 exact nine-collection/47-AC plan Review | D12 command Schema/Dart parity is resolved, but repository Done is blocked by an unrelated Route golden; named API-DEC-01/03, TypeScript/query parity, OD-09/11, Identity, Event projection, effects/repair seams and production evidence remain blocked; no authoritative runtime | Resolve/disposition the Route golden independently, obtain named API verdicts, then complete remaining reviews/dependencies; only afterwards may a separately Approved ECL-03C runtime slice be considered |
...
46. **BCK-01-AC-46:** BCK-02 v2.4.48 (Approved v2.4 semantics), BCK-02-A1
    Draft v1.0 и BCK-09 Review v1.6 представлены с их фактическими
