...
- Specialist review:
  [BCK09-REV-01 v0.7 — parity/evidence Hold; signatures Pending](BACKEND_EVENT_BOOKING_SPECIALIST_REVIEW_PACKAGE.md)
...
trimming are forbidden after validation. Dart/TypeScript golden vectors and
independent Security evidence remain mandatory before runtime.
...
no endpoint may be implemented until TypeScript/query/hash parity and all
independent evidence gates are complete.
...
| ECL-03C | Exact transaction-core plan v1.6 | Review | Not authorized |
...
|---|---|---|---|---|
| BCK09-OD-01 | Deferred/Open | Booking + Architecture | ECL-03C v1.6 is the only first executable candidate; grant separate executable authorization | No backend/product runtime |
| BCK09-OD-02 | Named API decisions Accepted; evidence open | API Platform + Booking + Security + BCK-05 | Preserve Booking v1/D11; prove TypeScript/query/hash parity and independent stage/Security evidence | No mutation endpoint |
