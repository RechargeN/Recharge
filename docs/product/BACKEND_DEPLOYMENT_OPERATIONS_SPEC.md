...
- Parent architecture: [BCK-01 v0.4.28](RECHARGE_BACKEND_MASTER_SPEC.md) (Review)
...
- Security/privacy boundary: [BCK-04 v0.4.16](BACKEND_SECURITY_PRIVACY_SPEC.md) (Draft; OD-01/09 Proposed; OD-07 and BCK05-OD-02 Accepted)
...
- Release/provenance evidence: [BCK05-OD07-REL-01](BACKEND_RELEASE_PROVENANCE_PROMOTION_MODEL.md) v0.2.1 (Accepted baseline with controls)
- Release owner decision: [BCK05-OD07-DEC-01](BACKEND_RELEASE_PROVENANCE_PROMOTION_OWNER_DECISION.md) v0.2 (Accepted)
...
- retained BCK-05 Draft, executable release/IAM/policy/provider evidence,
  D1/G1, separately Approved R0/R1 and every production gate;
- created no workflow, GitHub setting, release, attestation, WIF/cloud resource,
  deployment, runtime or `main` merge authority.
...
- recorded the exact BCK05-OD02-DEC-01 v0.2 owner verdict and accepted
  `BCK05-IAM-A1-ENV-WIF-v1` with all stated controls;
- preserved BCK-05 Draft, D1/G1/R1, executable claim/permission/binding/JIT
  evidence and every other operations decision as independent gates;
- created no GitHub/GCP mutation, IAM binding, secret, credential, deployment,
  runtime or `main` merge authority.
...
- completed the review-ready `OD07-A1-EU-MR-v1` resource/location matrix and
  added the exact unsigned `OD07-DEC-01` owner-decision contract;
- distinguished European location-controlled customer-content resources from
  Firebase Authentication/global service processing and retained the qualified
  Legal/Privacy production gate;
- kept OD-07 Proposed, BCK-05 Draft, D1/G1/R1 blocked and cloud runtime Absent.

### v0.2.17 — 2026-08-24

- recorded the exact `BCK05-OD01-DEC-01` owner verdict and advanced only
  `BCK05-OD-01` to Accepted at baseline v0.3.3 with controls;
- retained BCK-05 Draft and OD-02/03/04/05/07/08 Proposed, with OD-07 and the
  complete D1/G1 set still blocking R1;
- required a fresh dependency audit/reachability review before R1 and updated
  BCK-01/BCK-02 traceability to v0.4.22/v2.4.26.
...
- Firestore **Standard edition, Native mode, `(default)`, `eur3`**;
- Functions **2nd gen `europe-west1`**, explicitly configured in backend and
  callable clients;
- runtime media Storage **regional `europe-west1`**; disabled recovery/export
  bucket **`EU` multi-region**;
...
- location-controlled customer content is European; Authentication/global
  services and mandatory/global logs are disclosed separately and remain
  production-disabled until their Security/Privacy/Legal gates;
...
| BCK05-OD-02 | Accepted — `BCK05-IAM-A1-ENV-WIF-v1` with controls | Platform Security/Operations | exact verdict recorded in [BCK05-OD02-DEC-01 v0.2](BACKEND_IAM_WORKLOAD_IDENTITY_OWNER_DECISION.md) against [IAM model v0.2](BACKEND_IAM_WORKLOAD_IDENTITY_MODEL.md); no runtime authority | executable claims, permissions, bindings, JIT/roster, BCK-05/G1/R1 remain separate gates |
...
| BCK05-OD-07 | Accepted with controls | Release Operations + Platform Security | [BCK05-OD07-REL-01 v0.2.1](BACKEND_RELEASE_PROVENANCE_PROMOTION_MODEL.md) and exact [BCK05-OD07-DEC-01 v0.2](BACKEND_RELEASE_PROVENANCE_PROMOTION_OWNER_DECISION.md) accept `BCK05-REL-A1-DUAL-PROV-v1`; executable/policy/provider evidence remains pending | separately Approved executable R0 |
...
At v0.2.23 the following remain absent:
...
3. revalidate OD-07 and BCK05-OD-02 before provisioning, then resolve
   BCK05-OD-03/04/05/07/08 evidence;
