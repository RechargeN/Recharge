...
void main() {
  ...
  group('restart recovery', () {
    test('a fresh datasource instance over the same store sees what a prior '
        'instance committed — active version, receipt replay, and a pending '
        'moderation request all survive', () async {
      ...
      final PublishedCollectionVersion? active = await restarted.activeVersion(
        'col-1',
      );
  ...
  group('partial-write recovery', () {
    test('a crash writing the version record itself is caught by that '
        'write\'s own readback check — no pointer ever ends up naming an '
        'unreadable version', () async {
    ...
    test('CLG-PST-01 review finding: a crash writing the pointer itself — the '
        'record CLG-PST-01 never verified — leaves the previous active '
        'version exactly as it was, not partially overwritten', () async {
      final _FaultInjectingStore store = _FaultInjectingStore(
        const SecureCollectionPublicationStore(FlutterSecureStorage()),
      );
      final CollectionPublicationLocalDatasource datasource =
      ...
      final PublishedCollectionVersion? before = await datasource.activeVersion(
        'col-1',
      );
      ...
      // pointer write.
      // Simulate a storage layer that reports success but silently loses
      // the pointer write — stricter than a normal thrown write failure.
      store.dropWriteForKey = 'collection_publication_v1.pointer.col-1';
      await expectLater(
        datasource.publish(
      ...
      final PublishedCollectionVersion? after = await datasource.activeVersion(
        'col-1',
      );
      expect(after, isNotNull);
      expect(after!.collectionVersionId, before!.collectionVersionId);

      // Receipt was already durable. Exact replay completes the missing
      // pointer flip and does not create another logical publication.
      store.dropWriteForKey = null;
      final CollectionPublishReceipt replay = await datasource.publish(
        _bundle(versionId: 'v2', publishAttemptId: 'attempt-2'),
        actorId: 'actor-1',
      );
      expect(
    ...
    test('a corrupt current pointer falls back to the last-known-good '
        '.previous pointer instead of surfacing as data loss', () async {
      const CollectionPublicationStore store = SecureCollectionPublicationStore(
        FlutterSecureStorage(),
      );
      final CollectionPublicationLocalDatasource datasource =
      ...
      // A missing primary is also recoverable when `.previous` exists.
      await store.delete('collection_publication_v1.pointer.col-1');
      expect(
        (await datasource.activeVersion('col-1'))!.collectionVersionId,
        'v1',
      );

      // Recreate a corrupt primary, then commit v3. The corrupt bytes must
      // not overwrite the good `.previous` backup during that commit.
      await store.write(
        'collection_publication_v1.pointer.col-1',
        '{still not valid json',
      );
      await datasource.publish(
        _bundle(versionId: 'v3', publishAttemptId: 'attempt-3'),
        actorId: 'actor-1',
      );
      await store.write(
        'collection_publication_v1.pointer.col-1',
    ...
    test('a removal receipt failure leaves the prior version active and an '
        'exact retry completes the prepared command', () async {
      final _FaultInjectingStore store = _FaultInjectingStore(
        const SecureCollectionPublicationStore(FlutterSecureStorage()),
      );
      final CollectionPublicationLocalDatasource datasource =
      ...
      // The removal's own receipt write fails after immutable staging but
      // before the pointer marker.
      ...
      // Exact retry reuses the staged immutable version and then commits it.
      store.failWriteForKey = null;
      final CollectionPublishReceipt receipt = await datasource.removeItemsOnly(
        command,
      );
      expect(receipt.outcome, CollectionPublishOutcome.created);
      expect((await datasource.activeVersion('col-1'))!.bundle.items, isEmpty);
    });

    test(
      'a publish receipt failure never exposes the staged version',
      () async {
        final _FaultInjectingStore store = _FaultInjectingStore(
          const SecureCollectionPublicationStore(FlutterSecureStorage()),
        );
        ...
        store.failWriteForKey =
            'collection_publication_v1.receipt.publish.actor-1.attempt-1';
        ...
        store.failWriteForKey = null;
        final CollectionPublishReceipt recovered = await datasource.publish(
          _bundle(),
          actorId: 'actor-1',
        );
        expect(recovered.outcome, CollectionPublishOutcome.created);
        expect(await datasource.activeVersion('col-1'), isNotNull);
      },
    );

    test(
      'a submit receipt failure never exposes a pending moderation request',
      () async {
        final _FaultInjectingStore store = _FaultInjectingStore(
          const SecureCollectionPublicationStore(FlutterSecureStorage()),
        );
        ...
        store.failWriteForKey =
            'collection_publication_v1.receipt.review.actor-1.attempt-1';

        await expectLater(
          datasource.submitForReview(_bundle(), actorId: 'actor-1'),
          throwsA(isA<CollectionPublicationException>()),
        );
        expect(await datasource.pendingRequests(), isEmpty);

        store.failWriteForKey = null;
        final CollectionPublishReceipt recovered = await datasource
            .submitForReview(_bundle(), actorId: 'actor-1');
        expect(recovered.outcome, CollectionPublishOutcome.pendingReview);
        expect(await datasource.pendingRequests(), hasLength(1));
      },
    );

    test(
      'a durable submit receipt repairs a missing moderation marker on replay',
      () async {
        final _FaultInjectingStore store = _FaultInjectingStore(
          const SecureCollectionPublicationStore(FlutterSecureStorage()),
        );
        ...
        store.failWriteForKey =
            'collection_publication_v1.moderation.attempt-1';
        ...
        store.failWriteForKey = null;
        final CollectionPublishReceipt replay = await datasource
            .submitForReview(_bundle(), actorId: 'actor-1');
    ...
    test(
      'an accepted review is hidden until its sealed decision is durable',
      () async {
        final _FaultInjectingStore store = _FaultInjectingStore(
          const SecureCollectionPublicationStore(FlutterSecureStorage()),
        );
        ...
        store.failWriteForKey = null;
        final (
          CollectionModerationRequest request,
          PublishedCollectionVersion? active,
    ...
    test(
      'replaying an older receipt never rolls a newer pointer backwards',
      () async {
        const CollectionPublicationStore store =
            SecureCollectionPublicationStore(FlutterSecureStorage());
        final CollectionPublicationLocalDatasource datasource =
            CollectionPublicationLocalDatasource(
              idGenerator: _SequentialIdGenerator(),
              store: store,
  ...
  group('archive tombstone', () {
    test('an audit failure leaves the collection active until retry', () async {
      final _FaultInjectingStore store = _FaultInjectingStore(
        const SecureCollectionPublicationStore(FlutterSecureStorage()),
      );
      final CollectionPublicationLocalDatasource datasource =
      ...
      await datasource.publish(_bundle(), actorId: 'actor-1');
      store.failWriteForKey =
          'collection_publication_v1.audit.col-1.archive.actor-1.arch-1';

      await expectLater(
        datasource.archive('col-1', actorId: 'actor-1', requestId: 'arch-1'),
        throwsA(isA<CollectionPublicationException>()),
      );
      expect(await datasource.activeVersion('col-1'), isNotNull);
    ...
    test('CLG-PST-01 review finding: a retried archive() after a sink failure '
    ...
    test('a republish after an archive supersedes the old tombstone — the '
        'fresh version reads back as active, not archived', () async {
      const CollectionPublicationStore store = SecureCollectionPublicationStore(
        FlutterSecureStorage(),
      );
      final CollectionPublicationLocalDatasource datasource =
  ...
  group('audit trail', () {
    test('command type and actor isolate audit identities', () async {
      const CollectionPublicationStore store = SecureCollectionPublicationStore(
        FlutterSecureStorage(),
      );
      final CollectionPublicationLocalDatasource datasource =
      ...
      final Map<String, String> records = await store.readAllWithPrefix(
        'collection_publication_v1.audit.col-1.',
      );
      expect(records, hasLength(2));
      expect(
        records.keys,
        containsAll(<String>[
          'collection_publication_v1.audit.col-1.submit.actor-1.attempt-1',
          'collection_publication_v1.audit.col-1.publish.actor-1.attempt-1',
        ]),
      );
    });

    test('publish, removeItemsOnly and archive each leave an audit record '
        'naming the real actor and, for removal, the diff', () async {
      const CollectionPublicationStore store = SecureCollectionPublicationStore(
        FlutterSecureStorage(),
      );
      final CollectionPublicationLocalDatasource datasource =
      ...
      final Map<String, String> afterPublish = await store.readAllWithPrefix(
        'collection_publication_v1.audit.col-1.',
      );
      ...
      final String removalAudit = (await store.read(
        'collection_publication_v1.audit.col-1.removal.actor-2.removal-1',
      ))!;
      ...
      final String archiveAudit = (await store.read(
        'collection_publication_v1.audit.col-1.archive.actor-3.arch-1',
      ))!;
      expect(archiveAudit, contains('"actor_id":"actor-3"'));
      expect(archiveAudit, contains('"command_type":"archive"'));
    });

    test('a corrupt audit record is not read by any production path — it '
        'never blocks reading the active version', () async {
      const CollectionPublicationStore store = SecureCollectionPublicationStore(
        FlutterSecureStorage(),
      );
      final CollectionPublicationLocalDatasource datasource =
  ...
  group('corrupt-record isolation', () {
    test('a corrupt active-version record surfaces persistenceUnavailable '
      ...
      final PublishedCollectionVersion? good = await datasource.activeVersion(
        'col-good',
      );
    ...
    test('a corrupt moderation request is skipped by pendingRequests(), other '
        'pending requests still come back', () async {
      const CollectionPublicationStore store = SecureCollectionPublicationStore(
        FlutterSecureStorage(),
      );
      final CollectionPublicationLocalDatasource datasource =
    ...
    test('a tampered content_hash is treated the same as malformed JSON — the '
        'record is corrupt, not silently trusted', () async {
      const CollectionPublicationStore store = SecureCollectionPublicationStore(
        FlutterSecureStorage(),
      );
      final CollectionPublicationLocalDatasource datasource =
  ...
  group('typed-failure wrapping', () {
    test('a raw storage exception from pendingRequests() surfaces as '
        'persistenceUnavailable, not an unhandled exception', () async {
    ...
    test('a raw storage exception reading a moderation request in decide() '
        'surfaces as persistenceUnavailable', () async {
      final _FaultInjectingStore store = _FaultInjectingStore(
        const SecureCollectionPublicationStore(FlutterSecureStorage()),
      );
      final CollectionPublicationLocalDatasource datasource =
    ...
    test('a raw storage exception inside archive() surfaces as '
        'persistenceUnavailable', () async {
      final _FaultInjectingStore store = _FaultInjectingStore(
        const SecureCollectionPublicationStore(FlutterSecureStorage()),
      );
      final CollectionPublicationLocalDatasource datasource =
